...
export function derivePortfolio(input: PortfolioDerivationInput): PortfolioDerivation {
  ...
  return {
    ...
    summary,
    allocation,
    sortedHoldings: sortHoldings(holdingsFull, input.sortH),
    sortedWatch: sortWatch(watchFull, input.sortW),
  };
}

export function deriveHoldingsFull(
  holdings: HoldingItem[],
  quotes: Record<string, Quote>
): HoldingFull[] {
...
export function deriveWatchFull(
  watch: WatchItem[],
  quotes: Record<string, Quote>
): WatchFull[] {
  return watch.map((w) => {
