...
// status banner — drives all of section IV/VI/VII visible states
function StatusBanner({ P, S, surface, m, st, fk }) {
  const wrap = (tone, icon, text, actions, busy) => {
    ...
    return (
      <div style={{ background: soft, borderRadius: S.radius, padding: `${S.pad * 0.7}px ${S.pad * 0.8}px`,
        ...
        alignItems: surface === "mobile" && actions ? "flex-start" : "center", gap: 12 }}>
  ...
  if (st === "synced") return wrap("ok", "CircleCheck", "已同步 · " + m.state.lastSync + " · 来自 " + m.state.source,
    <SyncBtn P={P} S={S} variant="ghost" onClick={m.markEdited}>试一下：修改本机配置</SyncBtn>);

  if (st === "pending") return wrap("warn", "Clock", "有本机更改，等待自动同步",
    <SyncBtn P={P} S={S} variant="primary" icon="RefreshCw" full={surface === "mobile"} onClick={m.syncNow}>立即同步</SyncBtn>);

  if (st === "syncing") return wrap("info", "RefreshCw", "正在同步…", null, true);
